...
/**
 * Escapes special characters for safe inclusion in ZCQL query strings.
 * Prevents ZCQL / SQL injection vulnerabilities.
 * 
 * @param {string} val 
 * @returns {string}
 */
function escapeZcqlString(val) {
...
/**
 * Production ZCQL Query Execution Engine.
 * Executes live ZCQL queries strictly against the Zoho Catalyst Data Store.
 * 
 * @param {object} app - Catalyst App instance
 * @param {string} query - ZCQL query string
 * @returns {Promise<Array<object>>} Flattened result rows from Data Store
 */
async function executeQuery(app, query) {
  if (!query) return [];

  if (!app || typeof app.zcql !== 'function') {
    console.error('[Database Engine Error] Invalid or missing Catalyst App instance.');
    throw new Error('Database Error: Catalyst App context is required for Data Store ZCQL execution.');
  }
  ...
  try {
    ...
    console.error(`[ZCQL Execution Error] Query failed against Data Store: "${query}" - ${err.message}`);
    throw new Error(`Data Store Query Failure: ${err.message}`);
  }
}
