...
async function getCaseDetail(req, res) {
  try {
    ...
    const queries = {
      caseMasterPart1: `SELECT CaseMaster.ROWID, CaseMaster.fir_number, CaseMaster.crime_registered_date, CaseMaster.incident_from_date, CaseMaster.incident_to_date, CaseMaster.info_received_date, CaseMaster.latitude, CaseMaster.longitude, CaseMaster.place_of_occurrence, CaseMaster.summary_of_facts, CaseMaster.fir_status, District.district_name, Unit.unit_name, Court.court_name FROM CaseMaster LEFT JOIN District ON CaseMaster.district_id = District.ROWID LEFT JOIN Unit ON CaseMaster.unit_id = Unit.ROWID LEFT JOIN Court ON CaseMaster.court_id = Court.ROWID WHERE CaseMaster.ROWID = ${caseId}`,
      ...
      arrests: `SELECT Arrest.ROWID, Arrest.accused_id, Arrest.event_type, Arrest.date_time, Arrest.place, Arrest.remarks, Employee.employee_name AS arresting_officer_name FROM ArrestSurrender Arrest LEFT JOIN Employee ON Arrest.arresting_officer_id = Employee.ROWID WHERE Arrest.case_id = ${caseId}`,
    ...
    const caseMasterPart1List = resolvedData.caseMasterPart1;
    const caseMasterPart2List = resolvedData.caseMasterPart2;
    ...
    const caseObjPart2 = caseMasterPart2List && caseMasterPart2List.length > 0 ? caseMasterPart2List[0] : {};
